...
main_df = None # student's dataimport importlib, sys, json, io, time, traceback, itertools, re, os, math
from datetime import datetime, timedelta
from collections import namedtuple
from matplotlib import pyplot as plt
from io import StringIO, BytesIO
        ...
        for col in main_df.columns:
            ...
            for i in range(len(expected)):
                if expected.iat[i] != actual.iat[i]:
                    if isinstance(expected.iat[i], (np.float64, np.float32, float)) and isinstance(actual.iat[i], (np.float64, np.float32, float)): # first check for numpy floats
